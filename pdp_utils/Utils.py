...
route_cost_cache = {}
GLOBAL_CARGO = None  # Global variable to hold Cargo


from functools import lru_cache

def init_globals(problem):
    global GLOBAL_CARGO
    GLOBAL_CARGO = problem['Cargo']


@lru_cache(maxsize=None)
def get_sorted_route_data(route_tuple):
    """
    Given a candidate route (as a tuple of 0-indexed calls) and the Cargo matrix,
    perform the heavy sorting operations and return the computed values.
    
    Returns:
      sortRout: Sorted version of the route (using mergesort).
      I: The indices that would sort the route.
      Indx: The inverse permutation of I.
      PortIndex: The computed port index array.
    """
    # Convert the tuple to a NumPy array.
    route_arr = np.array(route_tuple, dtype=int)
    sortRout = np.sort(route_arr, kind='mergesort')
    I = np.argsort(route_arr, kind='mergesort')
    Indx = np.argsort(I, kind='mergesort')
    PortIndex = GLOBAL_CARGO[sortRout, 1].astype(int)
    # For every other element (assumed pickup positions), replace with the pickup node.
    PortIndex[::2] = GLOBAL_CARGO[sortRout[::2], 0]
    # Adjust for 0-indexing (assuming Cargo nodes are 1-indexed)
    PortIndex = PortIndex[Indx] - 1
    return sortRout, I, Indx, PortIndex
...
def cost_function_light(Solution, problem, vehicle_idx):
    ...
    if vehicle_idx == num_vehicles:
        ...
        NotTransportCost = np.sum(GLOBAL_CARGO[route_arr, 3]) / 2
        return NotTransportCost
    elif N > 0:
        route_tuple = tuple(route_arr.tolist())
        sortRout, I, Indx, PortIndex = get_sorted_route_data(route_tuple)

        Diag = TravelCost[vehicle_idx, PortIndex[:-1], PortIndex[1:]]
        FirstVisitCost = FirstTravelCost[vehicle_idx, int(GLOBAL_CARGO[route_arr[0], 0] - 1)]
